...
procedure InformarPromedioDeEdad(a:arbol);
  ...
  procedure sumatoria(a:arbol; var suma:integer);
  begin
    if (a <> nil) then begin
...
begin
  ...
  cantidad(a,cant);
  sumatoria(a,suma);
  writeln('el promedio de edades es: ',suma div cant);
end;
procedure InformarNumerosSociosOrdenCreciente(a:arbol);
begin
  if (a <> nil) then begin
    writeln('El numero de socio de la persona es: ',a^.dato.numero);
...
procedure InformarCantidadSociosEnRango(a:Arbol);
  procedure enrango(a:arbol;i:integer;s:integer);
  begin
    if (a <> nil) then begin
      if (a^.dato.numero >= i) and (a^.dato.numero <= s) then begin
        writeln('El numero de socio de la persona es: ',a^.dato.numero);
        enrango (a^.HI,i,s);
        enrango (a^.HD,i,s);
      end;
...
begin
  i:= 2;
  s:= 6;
  writeln('Las personas dentro del rango es');
  enrango(a,i,s);
end;
